...
void loop() {
  ...
  if (MIDI1.read()) {
    ...
    if (type != midi::SystemExclusive) {
      ...
      MIDI1.send(static_cast<midi::MidiType>(type), data1, data2, channel);
      MIDI6.send(static_cast<midi::MidiType>(type), data1, data2, channel);
   ...
   */
      ...
      char c[256];
      sprintf(c,"%d %d %d",data1,data2,channel);
      Serial.println(c);
